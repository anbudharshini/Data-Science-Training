...
// 6. $lookup to show each order with the product name and price
db.orders.aggregate([{$lookup:{from:"products",localField:"product_id",foreignField:"product_id",as:"product_info"}},
{$unwind:"$product_info"},
{$project:{_id:0,order_id:1,customer:1,quantity:1,order_date:1,product_name:"$product_info.name",price:"$product_info.price"}}])
...
{$unwind:"$product"},
{$group:{_id:"$customer",total_spent:{$sum:{$multiply:["$quantity","$product.price"]}}}}])

// 8. List all orders along with category of the product
db.orders.aggregate([{$lookup:{from:"products",localField:"product_id",foreignField:"product_id",as:"product"}},
{$unwind:"$product"},
{$project:{_id:0,order_id:1,customer:1,category:"$product.category"}}])
...
db.orders.aggregate([{$lookup:{from:"products",localField:"product_id",foreignField:"product_id",as:"product"}},
{$unwind:"$product"},
{$group:{_id:"$product.category",total_sales:{$sum:{$multiply:["$quantity","$product.price"]}}}}])

// Aggregations & Grouping
// 11. Count how many units of each product have been sold
db.orders.aggregate([{$group:{_id:"$product_id",units_sold:{$sum:"$quantity"}}}])
...
db.orders.aggregate([{$lookup:{from:"products",localField:"product_id",foreignField:"product_id",as:"product"}},
{$unwind:"$product"},
{$project:{customer:1,total:{$multiply:["$quantity","$product.price"]}}},
{$sort:{total:-1}},
{$limit:1}])

// 14. List the top 3 products based on number of orders
db.orders.aggregate([{$group:{_id:"$product_id",order_count:{$sum:1}}},
{$sort:{order_count:-1}},
{$limit:3}])

// 15. Determine which day had the highest number of orders
db.orders.aggregate([{$group:{_id:"$order_date",count:{$sum:1}}},
{$sort:{count:-1}},
{$limit:1}])

// Advanced Use Cases
// 16. Add a new customer who hasn't placed any orders. Query customers without orders.
db.customers.insertOne({name:"Rahul Kumar"})
db.customers.aggregate([{$lookup:{from:"orders",localField:"name",foreignField:"customer",as:"orders"}},
{$match:{orders:{$size:0}}}])

// 17. Add more orders and find customers who have placed more than one order
db.orders.insertMany([{order_id:5006,customer:"Ravi Shah",product_id:1003,quantity:1,order_date:new Date("2024-07-06")},
{order_id:5007,customer:"Sneha Mehta",product_id:1005,quantity:1,order_date:new Date("2024-07-06")}])
db.orders.aggregate([{$group:{_id:"$customer",order_count:{$sum:1}}},
{$match:{order_count:{$gt:1}}}])

// 18. Find all products that have never been ordered
db.products.aggregate([{$lookup:{from:"orders",localField:"product_id",foreignField:"product_id",as:"orders"}},
{$match:{orders:{$size:0}}}])
...
db.products.aggregate([{$project:{_id:0,product_id:1,name:1,inventory_value:{$multiply:["$price","$stock"]}}},
{$group:{_id:null,total_inventory_value:{$sum:"$inventory_value"}}}])
